...
const DebugForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const pepe = await axios.get(`https://api.stackexchange.com/2.2/search?order=desc&sort=activity&tagged=${tag}&intitle=${words}&site=stackoverflow`).then((res)=> {
...
<Section>
 ...
 {tags.map(x => <option value={x} onSelect={(x =>  setTag(x))} key={x}>{x}</option>)}
   </select>
   </div>
</Section>
   <Title>Click on the duck to ask a question</Title>
   <SvgRubberDuck style={{cursor: "pointer"}} onClick={()=>showForm(listenStatus)}   />
   <Section id="enquiry" style={{visibility: "hidden"}}>
   <label>
     Input:
     <input style ={{fontSize: "2em", textAlign: "center"}} className="textarea" type="text" id="transcript"
